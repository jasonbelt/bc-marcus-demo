...
Z bc_SharedMemory_create(STACK_FRAME Z id) {
    unsigned int permission = 0666;
    unsigned int mask = IPC_CREAT;
...
void bc_SharedMemory_receive(STACK_FRAME art_DataContent result, Z port) {
...
void bc_SharedMemory_receiveAsync(STACK_FRAME Option_8E9F45 result, Z port) {
    int sid = semget((key_t) port, 1, 0666);
...
Unit bc_SharedMemory_send(STACK_FRAME Z destid, Z port, art_DataContent d) {
    int sid = semget((key_t) port, 1, 0666);
...
B bc_SharedMemory_sendAsync(STACK_FRAME Z destid, Z port, art_DataContent d) {
    int sid = semget((key_t) port, 1, 0666);
